class SignUpTopic < ActiveRecord::Base
  ...
  def self.import(row,session,id, line_ind)
    ...
    error_s += " 'Topic identifier' is missing, " if row[0] == nil or row[0].strip.empty?
    error_s += " 'Topic name' is missing, " if row[1] == nil or row[1].strip.empty?
    error_s += " 'Max choosers' is missing, " if row[2] == nil or row[2].strip.empty?
    error_s += " 'Topic Category' is missing, " if row[3] == nil or row[3].strip.empty?
